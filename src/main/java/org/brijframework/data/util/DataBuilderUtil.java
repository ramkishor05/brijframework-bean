...
public class DataBuilderUtil {
	...
	/**
	 * Find current object for field if default is true than create object if its not found
	 * @param instance
	 * @param index
	 * @param field
	 * @param isDefault
	...
	private static <T> T findCurrentFromObject(Object instance,String _keyPath,boolean isDefault) {
		...
		PropertyMeta property=PropertyMetaFactoryImpl.getFactory().getPropertyInfo(instance.getClass().getSimpleName()+"_"+_keyPath);
	...
	public static <T> T setPropertyPath(Object instance,String _keyPath, T _value,boolean isDefault) {
		Assertion.notEmpty(_keyPath, "Key should not be null or empty");
	...
	@SuppressWarnings("unchecked")
	public static <T> T getCurrentInstance(Object object,String _keyPath, boolean isDefault) {
		if(object instanceof Map) {
			return getCurrentFromMapped((Map<String, Object> )object,_keyPath,isDefault);
		}else {
			return getCurrentFromInstance(object, _keyPath,isDefault);
		}
	}
	...
	@SuppressWarnings("unchecked")
	public static <T> T getCurrentFromInstance(Object object,String _keyPath, boolean isDefault) {
		String[] keyArray=_keyPath.split(Constants.SPLIT_DOT);
		Object current=object;
		StringBuffer point=new StringBuffer();
		for (int i = 0; i < keyArray.length-1; i++) {
			...
			if(temp==null && isDefault) {
				Class<?> cls=getProperty(current.getClass(),key);
				temp=InstanceUtil.getInstance(cls);
	...
	@SuppressWarnings("unchecked")
	public static  <T> T getCurrentFromMapped(Map<String, Object> object,String _keyPath, boolean isDefault) {
		String[] keyArray=_keyPath.split(Constants.SPLIT_DOT);
		Object current=object;
		for (int i = 0; i < keyArray.length-1; i++) {
			String key = keyArray[i];
	...
	@SuppressWarnings("unchecked")
	public static  <T> T getSupperFromMapped(Map<String, Object> object,String _keyPath, boolean isDefault) {
		String[] keyArray=_keyPath.split(Constants.SPLIT_DOT);
		Object current=object;
	...
	public static  Class<?> getProperty(Class<?> _class, String _field) {
		if(Map.class.isAssignableFrom(_class)) {
			...
			return ArrayList.class;
		}else if(Set.class.isAssignableFrom(_class)) {
			return HashSet.class;
		}else if(Collection.class.isAssignableFrom(_class)) {
			return ArrayList.class;
		}else {
			...
			PropertyMeta property= PropertyMetaFactoryImpl.getFactory().getPropertyInfo(_class.getSimpleName(),_field);
			if(property!=null && property.getType()!=null && !property.getType().equals(Type.class)) {
	...
	@SuppressWarnings("unchecked")
	public static <T> T setProperty(Object superInstance,Object[] array,Integer index,Access access,Object value) {
	...
	public static <T> T setProperty(Object superInstance,Object[] array,Integer index,Access access,String key,Object value) {
		if(array.length<=index) {
			return null;
		}
		return setProperty(array[index],access,key,value);
	}
	
	@SuppressWarnings("unchecked")
	public static <T> T setProperty(Object superInstance,Set<Object> array,Integer index, Access access, String key,Object value) {
		int count=-1;
		for (Iterator<?> iterator = array.iterator(); iterator.hasNext();) {
			if(++count==index) {
				setProperty(iterator.next(), access, key, value);
			}
		}
		return (T) value;
	}
	...
	@SuppressWarnings("unchecked")
	public static <T> T setProperty(Object superInstance,List<Object> array,Integer index, Access access, String key,Object value) {
		if(array.size()>index) {
			setProperty(array.get(index), access, key, value);
		}else {
		}
		return (T) value;
	}
	...
	@SuppressWarnings("unchecked")
	public static <T> T setProperty(Object superInstance,Object current,Integer index,Access access, String key,Object value) {
		if(current instanceof Object[]) {
			if(key==null) {
				return setProperty(superInstance,(Object[]) current, index, access, value);
			}else {
				return setProperty(superInstance,(Object[]) current, index, access, key, value);
			}
		}else if(current instanceof List) {
			return setProperty(superInstance,(List<Object>) current, index, access, key, value);
		}else if(current instanceof Set) {
			return setProperty(superInstance,(Set<Object>) current, index, access, key, value);
		}
		return null;
	}
